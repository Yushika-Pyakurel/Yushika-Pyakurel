# Introducing Yushika Pyakurel

I'm a Bachelor in Information Management (BIM) student passionate about web development and software engineering. I enjoy building modern, user-friendly, and scalable applications while continuously expanding my knowledge of new technologies.
 
-  Passionate about Web Development and Software Engineering
-  Skilled in HTML, CSS, JavaScript, and React.js
- Experienced with Microsoft SQL Server and Database Management
- Proficient in Python Programming
-  Familiar with PHP and Java
-  Currently Learning Django and Backend Development
-  Exploring Full-Stack Development and Modern Web Technologies
-  Continuously Learning and Expanding Technical Skills
-  Aspiring Backend & Full-Stack Developer
- Dedicated to Building Scalable, Efficient, and User-Friendly Applications
- 
**Tools**
- Git
- GitHub
- VS Code
 
# Connect With Me
 
- GitHub: https://github.com/Yushika-Pyakurel

<p align="left">
  https://skillicons.dev/icons?i=html,css,js,react,php,java,python,django
</p>


 
